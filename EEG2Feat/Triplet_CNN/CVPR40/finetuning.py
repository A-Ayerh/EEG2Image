...
if __name__ == '__main__':
    ...
    ckpt_lst = natsorted(glob('EXPERIMENT_{}/checkpoints/eegfeat_all_0.9720982142857143.pth'.format(experiment_num)))
    ...
    model.module.fc  = nn.Sequential(
                                ...
                                nn.LeakyReLU(),
                                nn.Dropout(p=0.1),
                                nn.Linear(in_features=512, out_features=config.thoutviz_classes, bias=False),
                                nn.Softmax(dim=1)
                                ).to(config.device)
