...
if __name__ == '__main__':
    ...
    print(x_train_eeg.shape, x_train_image.shape, train_labels.shape, train_subjects.shape)
    ...
    train_subjects  = torch.from_numpy(train_subjects).long()#.to(device)

    train_data       = EEGDataset(x_train_eeg, x_train_image, train_labels, train_subjects)
    ...
    val_subjects  = torch.from_numpy(val_subjects).long()#.to(device)

    print(x_val_eeg.shape, x_val_image.shape, val_labels.shape, val_subjects.shape)
    print('Total number of classes: {}'.format(len(np.unique(val_labels))), np.unique(val_labels))

    val_data       = EEGDataset(x_val_eeg, x_val_image, val_labels, val_subjects)
