...
class EEG2ImageDataset(Dataset):
    def __init__(self, path, resolution=None, **super_kwargs):
        ...
        if is_cnn == False:
            ...
            eegckpt   = 'eegbestckpt/eegfeat_lstm_all_0.9665178571428571.pth'
            ...
            ## Loading Pre-trained EEG CNN Encoder #####
            self.eeg_model = EEGCNNFeatNet(input_shape=config.input_shape, n_features=config.feat_dim, projection_dim=config.projection_dim).to(config.device)
            self.eeg_model = torch.nn.DataParallel(self.eeg_model).to(config.device)
            eegckpt   = 'eegbestckpt/eegfeat_cnn_all_0.9720982142857143.pth'
            eegcheckpoint = torch.load(eegckpt, map_location=config.device)
            self.eeg_model.load_state_dict(eegcheckpoint['model_state_dict'])
            print('Loading EEG CNN checkpoint from previous epoch: {}'.format(eegcheckpoint['epoch']))
            ########################################
        print("EEG Features:", np.array(self.eeg_feat))
        print("Labels:", np.array(self.labels))

        print('loading dataset...')
        print(self.dataset_path)
        file = ['dataset/eeg_imagenet40_cvpr_2017_raw/test2/n02510455_15145_4.npy']
