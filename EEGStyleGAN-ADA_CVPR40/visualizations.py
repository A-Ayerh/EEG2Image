...
class Umap:
    ...
    def plot(self, text_embed, labels, score, exp_type, experiment_num, epoch, proj_type):
        ...
        for i, label in enumerate(unique_labels):
            mask = labels == label
            ax.scatter(text_embed[mask, 0], text_embed[mask, 1], c=colors[i], label=label, alpha=0.7)
...
class K_means:
    ...
    def transform(self, embed, gt_labels):
        if len(embed) < self.n_clusters:
            print(f"Warning: Number of samples ({len(embed)}) is less than the number of clusters ({self.n_clusters}). Skipping K-means clustering.")
            return 0  # Return a default score or handle as needed
...
class TsnePlot:
    ...
    def plot(self, embedding, labels, score, exp_type, experiment_num, epoch, proj_type):
        ...
        for i, label in enumerate(unique_labels):
            mask = labels == label
            ax.scatter(reduced_embedding[mask, 0], reduced_embedding[mask, 1], c=colors[i], label=label, alpha=0.7)
        # ax.legend(loc='center left', fancybox=True, shadow=True, ncol=2, bbox_to_anchor=(1, 0.5))
        ax.legend(ncol=2)
        plt.tight_layout()
    ...
    def plot3d(self, embedding, labels, score, exp_type, experiment_num, epoch, proj_type):
        ...
        for i, label in enumerate(unique_labels):
            mask = labels == label
            ax.scatter(reduced_embedding[mask, 0], reduced_embedding[mask, 1], reduced_embedding[mask, 2], c=colors[i], label=label, alpha=0.7)
        # ax.legend(loc='center left', fancybox=True, shadow=True, ncol=2, bbox_to_anchor=(1, 0.5))
        ax.legend(ncol=2)
        plt.tight_layout()
...
def save_image(spectrogram, gt, experiment_num, epoch, folder_label):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('EXPERIMENT_{}/{}/{}_pred.pdf'.format(experiment_num, folder_label, epoch))
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('EXPERIMENT_{}/{}/{}_gt.pdf'.format(experiment_num, folder_label, epoch))
    plt.close('all')